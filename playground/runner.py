...
class ZBotRunner:
    ...
    def setup_training_config(self) -> None:
        ...
        self.rl_config = config_dict.create(
            ...
            num_updates_per_batch=1 if self.args.debug else 8,
            discounting=0.97,
            learning_rate=3e-4,
            entropy_cost=0.01,
            num_envs=1 if self.args.debug else 16384,
            batch_size=2 if self.args.debug else 512,
            max_grad_norm=1.0,
    ...
    def progress_callback(self, num_steps: int, metrics: dict) -> None:
        ...
        plt.errorbar(self.x_data, self.y_data,
                     yerr=self.y_dataerr, color="blue")
    ...
    def evaluate(self) -> None:
        ...
        for episode in range(self.args.num_episodes):
            ...
            for _ in range(self.args.episode_length):
                ...
                xyz = np.array(
                    state.data.xpos[self.eval_env.mj_model.body(zbot_constants.ROOT_BODY).id])
...
def main():
    ...
    parser.add_argument(
        '--env', type=str, default="ZbotJoystickFlatTerrain", help='Environment to run')
    parser.add_argument('--task', type=str,
                        default="flat_terrain", help='Task to run')
    ...
    parser.add_argument('--episode-length', type=int,
                        default=3000, help='Length of each episode')
    parser.add_argument('--x-vel', type=float, default=1.0,
                        help='X velocity command')
    parser.add_argument('--y-vel', type=float, default=0.0,
                        help='Y velocity command')
    parser.add_argument('--yaw-vel', type=float,
                        default=0.0, help='Yaw velocity command')
