    ...
    return config_dict.create(
        ...
        noise_config=config_dict.create(
            ...
            scales=config_dict.create(
                hip_pos=0.05,  # Increased from 0.03
                knee_pos=0.08,  # Increased from 0.05
                ankle_pos=0.12,  # Increased from 0.08
                joint_vel=2.0,  # Increased from 1.5
                gravity=0.1,  # Increased from 0.05
                linvel=0.2,  # Increased from 0.1
                gyro=0.3,  # Increased from 0.2
            ),
        ),
        reward_config=config_dict.create(
            scales=config_dict.create(
                ...
                lin_vel_z=-0.5,  # Added penalty for vertical movement
                ang_vel_xy=-0.3,  # Increased from -0.15
                orientation=-1.5,  # Increased from -1.0
                base_height=-0.2,  # Added penalty for height deviation
                # Energy related rewards
                torques=-3.0e-5,  # Increased from -2.5e-5
                action_rate=-0.02,  # Increased from -0.01
                energy=-0.001,  # Added small energy penalty
                # Feet related rewards
                feet_clearance=0.1,  # Added reward for foot clearance
                feet_air_time=2.0,
                feet_slip=-0.5,  # Increased from -0.25
                feet_height=0.1,  # Added reward for foot height
                feet_phase=1.0,
                # Other rewards
                stand_still=0.0,
                alive=1.0,  # Added alive bonus
                termination=-2.0,  # Increased from -1.0
                # Pose related rewards
                joint_deviation_knee=-0.2,  # Increased from -0.1
                joint_deviation_hip=-0.4,  # Increased from -0.25
                dof_pos_limits=-1.5,  # Increased from -1.0
                pose=-1.5,  # Increased from -1.0
            ),
            tracking_sigma=0.5,
            max_foot_height=0.15,  # Increased from 0.1
            base_height_target=0.5,
        ),
        push_config=config_dict.create(
            enable=True,
            interval_range=[3.0, 8.0],  # More frequent pushes
            magnitude_range=[0.2, 2.5],  # Stronger pushes
        ),
        lin_vel_x=[-1.0, 1.0],
        lin_vel_y=[-1.0, 1.0],
        ang_vel_yaw=[-1.0, 1.0],
...
class Joystick(zbot_base.ZbotEnv):
    ...
    def __init__(
        ...
        config_overrides: Optional[Dict[str,
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        push_interval_steps = jp.round(
            push_interval / self.dt).astype(jp.int32)
    ...
    def _get_termination(self, data: mjx.Data) -> jax.Array:
        ...
        return (
            fall_termination | jp.isnan(
                data.qpos).any() | jp.isnan(data.qvel).any()
        )
    ...
    def _cost_joint_deviation_hip(
        ...
        cost = jp.sum(
            jp.abs(qpos[self._hip_indices] -
                   self._default_pose[self._hip_indices])
        )
